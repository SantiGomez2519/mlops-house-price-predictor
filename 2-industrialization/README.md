...
| 3 | `2_3_data_preprocessed_train_feature_engineering.py` | `house_age`, scale numerics, ordinal `condition`, one-hot `location` | `src/data/data_featured_train.csv`, `src/models/feature_engineer.pkl` |
| 4 | `2_4_data_featured_train_training.py` | Tune models with GridSearchCV, keep the best | `src/models/model.pkl`, `src/models/model_config.json` |
| 5 | `2_5_data_raw_test_evaluation.py` | Apply saved artifacts to test (`transform`/`predict` only) | `src/data/data_featured_test_predictions.csv` |
